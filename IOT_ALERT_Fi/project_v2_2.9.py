...
'''def Credentials():
	...
	for Conf in Data:
		if Conf.startswith("account:"):
			Account = Conf.split(':')[1].split('\n')[0]
		if Conf.startswith("password:"):
			Password = Conf.split(':')[1].split('\n')[0]
		if Conf.startswith("domain:"):
			Imap = Conf.split(':')[1].split('\n')[0]
		if Conf.startswith("timer:"):
			Timer = int(Conf.split(':')[1].split('\n')[0])
		if Conf.startswith("interaction:"):
			Interaction = Conf.split(':')[1].split('\n')[0]
...
def PopUp(Subject,clr,State,Timer=0):
	...
	if State == "Pass":
		...
		Root.title("Error")
	WindowSpec = Canvas(Root,bg=clr,height=300,width=645)'''
	global list_subject
